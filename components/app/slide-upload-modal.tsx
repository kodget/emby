...
export function SlideUploadModal() {
  ...
  // Load sections when block is selected
  useEffect(() => {
    if (selectedBlock) {
      const token = localStorage.getItem('token');
      fetch(`http://localhost:8000/api/sections/?block=${selectedBlock}`, {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    dispatch(
      addUpload({
        id: uploadId,
        courseId: selectedSection || selectedBlock,
        moduleId: selectedSection || selectedBlock,
        materialId: "",
        title: title.trim(),
        fileName: file.name,
        pages: 0,
        ...
        >
          ...
          >
            ...
            {!canUpload ? (
              ...
              /* ── No curriculum ── */
              <div className="flex flex-col items-center gap-4 rounded-2xl border border-border bg-background/60 py-10 text-center">
                <p className="text-sm text-muted-foreground">No courses available. Please contact admin.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Section Selection (sub-blocks) */}
                {selectedBlock && sections.length > 0 && (
                  <div>
                    <label className="text-[11px] font-medium uppercase tracking-widest text-muted-foreground">
                      Section <span className="text-[9px] text-muted-foreground/60">(Optional)</span>
                    </label>
                    <div className="mt-1.5 grid grid-cols-2 gap-2">
